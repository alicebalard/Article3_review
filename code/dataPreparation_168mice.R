ExpeDF_003_4 <- readRDS(file = "../dataInfection/ExpeDF_003_4_Alice.rds")
ExpeDF_005 <- readRDS(file = "../dataInfection/ExpeDF_005_Alice.rds")
...
list.of.packages <- c(#"parasiteLoad",
                      "bbmle", "devtools", "optimx", # for bbmle it needs to be required(?)
...
### added here (was in main analyses code for article 2)
...
## remove useless factor levels
DSart2_conservative <- dropLevelsAllFactorsDF(DSart2_conservative)
art2SummaryDF_conservative <- dropLevelsAllFactorsDF(art2SummaryDF_conservative)

## verif correspondance
t(table(DSart2_conservative$Mouse_genotype[DSart2_conservative$dpi %in% 0],
        DSart2_conservative$infection_isolate[DSart2_conservative$dpi %in% 0]))
table(art2SummaryDF_conservative$infection_isolate, art2SummaryDF_conservative$Mouse_genotype)

## save objects for article3 review
art3FullDF <- DSart2_conservative
art3SummaryDF <- art2SummaryDF_conservative
  
## Rm useless data from environment
listObj <- objects()
listObj2rm <- listObj[!listObj %in% c("art3FullDF", "art3SummaryDF")]
